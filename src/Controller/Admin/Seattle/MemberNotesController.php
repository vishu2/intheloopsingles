...
class MemberNotesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
			...
			$memberNote = $this->MemberNotes->patchEntity($memberNote, $data);
			if ($this->MemberNotes->save($memberNote)) {
                $this->Flash->success(__('The member note has been saved.'));

                return $this->redirect(['controller' => 'Users','action' => 'memberdetail', base64_encode($this->request->getData('user_id'))]);
            }
