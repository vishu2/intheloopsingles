				...
				<tbody>
				   <?php $i=$this->Paginator->counter('{{start}}'); foreach ($events as $event): ?>
					<tr>
						...
						<td><?= $event->event_registrations ? $this->Html->link($event->event_registrations[0]->total, ['controller' => 'Events', 'action' => 'registered', $event->id]) : '' ?></td>
						
						<td>
							<?= h($event->created) ?>
						</td>
						<td class="text-muted">					  
							<?= $this->Html->link('<span class="fa fa-edit edit_btn"></span><span class="sr-only">' . __('edit') . '</span>', ['action' => 'edit', $event->id], ['escape' => false, 'title' => __('Edit')]) ?>						
