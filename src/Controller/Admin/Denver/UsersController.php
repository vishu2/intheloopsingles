...
class UsersController extends AppController {
	...
	$this->loadModel('Appointments');
		...
		$lead=$this->Leads->find('list', ['keyField' => 'id', 'valueField' => 'id'])->where(['location_id' => 2])->toArray();
		$appointments = $this->paginate($this->Appointments->find()->contain(['Leads' => ['LeadStatus'],'AppointmentStatus'])->where(['lead_id IN ' => $lead]));
		$events = $this->Events->find()->where(['Events.location_id ' => 2])->toArray();
		$leadcount= $this->Leads->find()->where(['Leads.lead_status_id'=> 1 ,'Leads.location_id'=>2 ])->count();
		$eventcount= $this->Events->find()->where(['Events.location_id'=>2 ])->count();
	...
	public function eventRegistration($id = null) {
		...
		$events=$this->Events->find()->where(['Events.location_id' => 2, 'Events.published' => 1, 
		...
		$travels=$this->Travels->find()->where(['Travels.location_id' => 2, 'Travels.published' => 1, 
	...
	public function member() {
		...
		$users = $this->paginate($this->Users->find()->where(['Users.role_id ' => 3, 'Users.location_id ' => 2])->order(['Users.id' => 'desc']));
		$this->set(compact('pageTitle', 'users'));
	}
