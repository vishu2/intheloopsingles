				...
				<tbody>
				   <?php $i=$this->Paginator->counter('{{start}}'); foreach ($travels as $travel): ?>
					<tr>
						...
						<td>
							<?php if(($travel->published)==1) { echo "Yes"; } else { echo "No";}?>
						</td>
						<td><?= $travel->travel_registrations ? $this->Html->link($travel->travel_registrations[0]->total, ['controller' => 'Travels', 'action' => 'registered', base64_encode($travel->id)]) : $this->Html->link('0', ['controller' => 'Travels', 'action' => 'registered', base64_encode($travel->id)]) ?></td>
						<td>
							<?= h($travel->created) ?>
						</td>
						<td class="text-muted">					  
							<?= $this->Html->link('<span class="fa fa-edit edit_btn"></span><span class="sr-only">' . __('edit') . '</span>', ['action' => 'edit', base64_encode($travel->id)], ['escape' => false, 'title' => __('Edit')]) ?>						
